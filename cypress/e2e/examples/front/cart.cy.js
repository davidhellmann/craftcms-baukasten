...
sizes.forEach((size) => {
  describe(`Cart on ${size} screen`, () => {
    ...
    it(`should be able to add a product to the cart`, function () {
      ...
      cy.get('div.line-item').its('length').should('be.gt', 0);
    });
    ...
    it(`should be able to remove a product from the cart`, function () {
      ...
      cy.get('div.line-item').then(($el) => {
        ...
        cy.get('#remove-trigger-' + lineItemId).click();
    ...
    it(`should be able to apply a discount code`, function () {
      ...
      cy.get('form#cart a.coupon-trigger').click();

      // Use the `15OFF` coupon code
      cy.get('form#cart input[name=couponCode]').type('15OFF');

      // Apply the coupon
      cy.get('form#cart input[type=submit].coupon-apply').click();

      // Check that the cart contains a discount row
      cy.get('form#cart div.discount-row').its('length').should('be.gt', 0);
    });
  });
});
